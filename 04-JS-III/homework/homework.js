...
function promedioResultadosTest(resultadosTest) {
  ...
  // Tu código:
  var suma = 0;
  for (cont = 0 ; cont < resultadosTest.length ; cont++){
    suma = suma + resultadosTest[cont];
...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
  var greater = numeros[0];
  for (cont = 1; cont < numeros.length ; cont++){
      if (numeros[cont] > greater){
        greater = numeros[cont];
      }
  }
  return greater;
}
...
function diaDeLaSemana(numeroDeDia) {
  ...
  if ((days.indexOf("Sábado")) === numeroDeDia || (days.indexOf("Domingo")) === numeroDeDia){
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  var arrayn = Array.from(String(n), Number);
  if (arrayn[0] === 9){
    return true;
  }else{
    return false;
